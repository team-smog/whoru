        ...
        {data.map((item) => (
          <div key={item.id} >
            <div className="py-2 border-b-[0.5px] border-gray-300">
              <div className="px-8 py-2" onClick={() => isOpenAccordion(item.id)}>
                <div className="text-[14px] text-text_color">{item.title}</div>
                <div className="text-[12px] text-gray-400 pl-1">{item.date}</div>
              </div>
              {openId === item.id && (
                <div className="mx-8 py-4 bg-gray-100 border-b-[0.5px] border-black">
                  ...
                  <div className="text-text_color px-4 pt-2 text-[12px]">{item.content}</div>
